...
class ProductControler:
    ...
    def get_product_by_name(self, name):
        sql         = "SELECT * FROM `products` INNER JOIN suppliers ON products.supplierId = suppliers.id INNER JOIN categores ON products.categoryId = categores.id INNER JOIN prosizes ON prosizes.proId = products.id WHERE products.proName like '%"+str(name.strip())+"%' GROUP BY `products`.`proName`;"
        myresult1       = connectdb.executeQuery(sql)
        result          = []
        for x in myresult1:
            sql2        = "SELECT * FROM `procolors` WHERE procolors.proId = "+str(x[0])+" ;"
