...
class CustomerController:
    ...
    def lock_customer_byId(self, id):
        sql         = "UPDATE `customers` SET `disable`=0 WHERE `customers`.`id` = "+str(id)+ " ;"
        myresult    = connectdb.RunQueryData(sql)
        return myresult
    
    def unlock_customer_byId(self, id):
        sql         = "UPDATE `customers` SET `disable`=1 WHERE `customers`.`id` = "+str(id)+ " ;"
        myresult    = connectdb.RunQueryData(sql)
        return myresult

    def update_customer_byId(self,customer: Customer):
        sql         = "UPDATE `customers` SET `email`='"+str(customer.email)+"',`password`='"+str(customer.password)+"',`image`='"+str(customer.image)+"',`gender`='"+str(customer.gender)+"',`lastName`='"+str(customer.lastName)+"',`firstName`='"+str(customer.firstName)+"',`city`='"+str(customer.city)+"',`distric`='"+str(customer.distric)+"',`street`='"+str(customer.street)+"',`address`='"+str(customer.address)+"',`phone`='"+str(customer.phone)+"',`birthDay`='"+str(customer.birthDay)+"',`disable`='"+str(customer.disable)+"' WHERE id = "+str(customer.id)+" ;"
        myresult    = connectdb.RunQueryData(sql)
        return myresult
    ...
    def add_customer(self, cus: Customer):
        sql         = "INSERT INTO `customers`(`id`,`email`, `password`, `image`, `gender`, `lastName`, `firstName`, `city`, `distric`, `street`, `address`, `phone`, `birthDay`, `disable`)VALUES (null,'"+str(cus.email)+"','"+str(cus.password)+"','"+str(cus.image)+"','"+str(cus.gender)+"','"+str(cus.lastName)+"','"+str(cus.firstName)+"','"+str(cus.city)+"','"+str(cus.distric)+"','"+str(cus.street)+"','"+str(cus.address)+"','"+str(cus.phone)+"','"+str(cus.birthDay)+"','1');"
